    ...
    internal class DDNAImpl : DDNABase
    {
        ...
        override internal void OnApplicationPause(bool pauseStatus)
        {
            if (pauseStatus)
            {
                ...
                if (Settings.OnPauseSendGameEndedStartedEvent)
                {
                    Logger.LogDebug("Sending 'gameEnded' event");

                    var gameEndedEvent = new GameEvent("gameEnded");
                    // .AddParam("clientVersion", this.ClientVersion)
                    // .AddParam("userLocale", ClientInfo.Locale);
                    //
                    // if (!string.IsNullOrEmpty(CrossGameUserID))
                    // {
                    //     gameEndedEvent.AddParam("ddnaCrossGameUserID", CrossGameUserID);
                    // }
                    //
                    // if (!String.IsNullOrEmpty(this.PushNotificationToken))
                    // {
                    //     gameEndedEvent.AddParam("pushNotificationToken", this.PushNotificationToken);
                    // }
                    //
                    // if (!String.IsNullOrEmpty(this.AndroidRegistrationID))
                    // {
                    //     gameEndedEvent.AddParam("androidRegistrationID", this.AndroidRegistrationID);
                    // }
                    // gameEndedEvent.AddParam("gameEndState", "paused");

                    RecordEvent(gameEndedEvent).Run();
                }
        ...
        {
            if (Settings.OnDestroySendGameEndedEvent)
            {
                Logger.LogDebug("Sending 'gameEnded' event");
                ...
                //     gameEndedEvent.AddParam("androidRegistrationID", this.AndroidRegistrationID);
                // }
                // gameEndedEvent.AddParam("gameEndState", "stopped");
